...
```math
h_{\ell+1}=h_{\ell}+\mathrm{MLP}(h_{\ell}+\mathrm{Attention}(h_{\ell}))
```

更常见的 pre-norm 写法是：

```math
u_{\ell}=h_{\ell}+\mathrm{Attention}(\mathrm{Norm}(h_{\ell}))
```

```math
h_{\ell+1}=u_{\ell}+\mathrm{MLP}(\mathrm{Norm}(u_{\ell}))
```
...
```math
a_t = \mathrm{Attention}(q_t, K_{\le t}, V_{\le t})
```
...
```math
\mathcal{E}_{topk} = \mathrm{TopK}(r, k=8)
```
