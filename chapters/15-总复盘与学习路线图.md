...
- 把全书的模型、系统、源码、部署和压测知识收束成可执行路线。
- 帮你检查自己是否已经具备读模型卡、估显存、读 scheduler、设计压测和写排障报告的能力。
- 给出 30 天行动路线，把阅读转化成源码阅读、实验验证和工程输出。
- 说明哪些能力可以迁移到其他模型和 serving 框架，哪些必须绑定版本和硬件重新验证。
...
| Qwen3.6 在具体机器上的实际 attention backend | `vllm/v1/attention/backends/` + runtime logs | backend selection | 待运行确认 | 否 |
| Qwen3.6 在具体机器上的实际 MoE runner / all-to-all backend | `vllm/model_executor/layers/fused_moe/` + runtime logs | backend selection | 待运行确认 | 否 |
...
读完整本书后，你可以用下面的 checklist 判断自己是否真正建立了 serving 心智模型：

| 能力 | 自测问题 | 合格标准 |
|---|---|---|
| 读模型卡 | Qwen3.6 的 35B / 3B activated / 262K / MoE / MTP 分别影响什么？ | 能把每个参数映射到显存、调度或性能问题 |
| 估显存 | KV cache、GDN state、MoE buffer、Vision Encoder、MTP lookahead 哪些随上下文增长？ | 能说出 FP8 只解决部分权重压力 |
| 读 scheduler | vLLM 如何决定本 step 算哪些 request、多少 token？ | 能解释 running / waiting、token budget、preemption |
| 读 backend | attention、GDN、MoE 分别应看哪些源码目录？ | 能从 profiler 结果反查源码路径 |
| 设计部署 | 单卡 FP8、BF16 TP=8、text-only、EP、MTP 分别适合什么场景？ | 能给出基线配置和回滚配置 |
| 设计压测 | 如何区分 TTFT 高、TPOT 高、吞吐低和 OOM？ | 能写出 workload 矩阵、指标表和验收标准 |
| 写排障报告 | 一次线上慢请求应该记录哪些证据？ | 能给出日志、指标、profiler、结论和下一步动作 |

## 30 天行动路线

| 时间 | 目标 | 产出 |
|---|---|---|
| 第 1 周 | 复现传统推理生命周期和 Qwen3.6 架构地图 | 一页 baseline 图：token → model → logits → sampling |
| 第 2 周 | 深入 KV cache、PagedAttention、scheduler | 一份显存预算表和一次 preemption / prefix cache 分析 |
| 第 3 周 | 深入 MoE、TP、EP、NCCL、GPU kernel | 一份多卡瓶颈分析：compute、HBM、NCCL、all-to-all |
| 第 4 周 | 跑部署/压测/排障闭环 | 一份压测报告：命令、workload、指标、结论、回滚方案 |

如果没有真实 GPU 环境，第 4 周也可以先完成设计稿：明确你会测哪些 workload、采哪些指标、如何判断 MTP/EP/prefix caching 是否有效。等拿到硬件后，再用同一模板补真实数据。
