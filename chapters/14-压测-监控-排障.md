...
- 把“慢、抖、OOM、吞吐不达标”拆成可观测、可复现、可回滚的工程问题。
- 给出压测矩阵、监控指标和 runbook，而不是只看平均 tokens/s。
- 帮你判断问题属于 scheduler、KV/GDN state、attention backend、MoE、NCCL、sampler 还是多模态路径。
- 区分必须在真实 GPU 环境验证的实验和可以先做的阅读/设计作业。
...
每次压测至少留下这样一份记录，避免后续只记得“好像更快了”：

| 字段 | 记录内容 |
|---|---|
| 环境 | GPU 型号、GPU 数量、NVLink/PCIe/IB 拓扑、驱动、CUDA、容器镜像 |
| 软件 | vLLM 版本、模型 repo、dtype、TP/EP/DP、是否 text-only、是否 MTP |
| 启动命令 | 完整 `vllm serve ...` 命令和关键环境变量 |
| workload | prompt_len、output_len、concurrency、是否共享 prefix、是否多模态 |
| 延迟 | TTFT / TPOT / E2E latency 的 p50 / p90 / p99 |
| 吞吐 | prompt tokens/s、generation tokens/s、request/s |
| 显存与 cache | GPU memory、kv_cache_usage_perc、preemption、num_gpu_blocks |
| 调度 | running、waiting、queue time、prefill time、decode time |
| MoE / 通信 | per-rank util、expert imbalance、all-to-all / all-reduce 时间 |
| 结论 | 当前瓶颈、推荐配置、风险、回滚命令 |

没有这张表，任何“打开 MTP 更快”“EP 更好”“FP8 能上生产”的结论都不应该进入容量规划。
...
| 优化动作 | 主要验收指标 | 有效信号 | 回退信号 |
|---|---|---|---|
| 打开 prefix caching | prefix hit rate、TTFT、prefill time | 命中 workload 的 TTFT 明显下降 | hit rate 低且 metadata / cache 压力上升 |
| 调大 batch token budget | throughput、queue time、TTFT | 吞吐上升且 P99 不恶化 | 长 prompt 抢占短请求，TTFT P99 上升 |
| 打开 MTP | TPOT、accepted tokens、KV usage | 低并发 TPOT 下降且吞吐不回退 | 高并发 throughput 下降或 preemption 增多 |
| 启用 EP / EPLB | per-rank util、all-to-all time、tokens/s | expert imbalance 降低，总吞吐上升 | all-to-all 成本超过 expert 计算收益 |
| 切 text-only 服务池 | GPU memory、KV capacity、TTFT | 显存余量增加，纯文本 SLA 更稳定 | 误接多模态请求或服务池路由混乱 |
| 降 max model len | KV usage、OOM 次数、业务召回 | OOM 消失且业务上下文仍够用 | 长上下文任务能力不可接受 |

真实 GPU 环境必须验证：吞吐、延迟、OOM、backend 选择、NCCL/EP、MTP 收益、多模态 encoder 成本。阅读或设计阶段可以先完成：指标定义、压测矩阵、runbook、回滚命令和源码路径映射。
...
| GDN backend / Qwen3Next / KV manager 的具体运行瓶颈需要 profiler 验证 | Nsight / runtime logs | N/A | 需按环境验证 | 否 |
| 当前机器上实际 attention backend、GDN backend、MoE runner | 运行日志 / profiler | N/A | 待运行确认 | 否 |
